<script lang="ts">
	...
	// --- PERBAIKAN KUNCI ---
	// Tambahkan $effect ini untuk mereaksikan perubahan pada 'data' prop.
	// Ini memastikan state di-reset saat Anda menavigasi antar profil.
	$effect(() => {
		projectBreakdown = data.initialAnalytics.projectBreakdown || [];
		taskBreakdown = data.initialAnalytics.taskBreakdown || [];
		productivityTimeline = data.initialAnalytics.productivityTimeline || [];
		// Reset filter ke default saat berpindah profil
		activeFilter = 'week';
	});
	...
	function createOrUpdateTimelineChart() {
		...
		const timeBuckets = [...new Set(productivityTimeline.map((d: any) => d.time_bucket))].sort();
		const projects = [...new Set(productivityTimeline.map((d: any) => d.project_name))];
		...
		const datasets = projects.map((projectName, index) => ({
			...
			data: timeBuckets.map((bucket) => {
				const entry = productivityTimeline.find(
					(d: any) => d.time_bucket === bucket && d.project_name === projectName
				);
	...
	function createOrUpdateProjectChart() {
		...
		const labels = projectBreakdown.map((p: any) => p.project_name);
		const data = projectBreakdown.map((p: any) => Number(p.total_focus_minutes) || 0);
		if (projectChartInstance) {
	...
	function createOrUpdateTaskChart() {
		...
		const labels = topTasks.map((t: any) =>
			t.task_title.length > 25 ? t.task_title.substring(0, 25) + '...' : t.task_title
		);
		const data = topTasks.map((t: any) => Number(t.total_focus_minutes) || 0);
		if (taskChartInstance) {
	...
	<div class="flex flex-col sm:flex-row items-center gap-6">
		...
		<div>
			...
			<p class="text-gray-500 text-center sm:text-left">
				{data.profile.email || 'Email tidak tersedia'}
			</p>
		</div>
	</div>
			...
			<div class="lg:col-span-2 bg-gray-50 p-4 rounded-lg">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t">
					...
					<div>
						...
						<div class="h-64 space-y-2 overflow-y-auto" bind:this={taskChartCanvas}>
							{#if isLoading}
