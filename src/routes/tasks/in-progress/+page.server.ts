// src/routes/tasks/in-progress/+page.server.ts
...
export const load: PageServerLoad = async ({ locals: { supabase, session } }) => {
	...
	// Ambil semua tugas yang ditugaskan ke user ini DAN berada di kolom 'In Progress'
	const { data: tasks, error } = await supabase
		...
		.eq('columns.name', 'In Progress'); // Filter langsung di database
